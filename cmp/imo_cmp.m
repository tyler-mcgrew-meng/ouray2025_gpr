...
air = readtable("cmp3_air-Point.csv",NumHeaderLines=3);
ref = readtable("cmp3_ref-Point.csv",NumHeaderLines=3);
...
L1r_x = ref.Position_m_; L1r_t = ref.Time_ns_;
L1s_x = air.Position_m_; L1s_t = air.Time_ns_;

[Rm, Rb] = polyfit(L1r_x, L1r_t, 1); Ref = Rm(1)*X+Rm(2);
[Sm, Sb] = polyfit(L1s_x, L1s_t, 1); Surf = Sm(1)*X+Sm(2);
Vs = 1/Sm(1); Vr = 1/Rm(1);
disp('Air wave:'); disp(Vs); 
disp('Refracted wave:'); disp(Vr);
